...
FROM
	Liste_tache LT
JOIN Tache_appartenant_a_liste TAL ON
	LT.ref_liste = TAL.ref_liste
JOIN Utilisateur U ON
	LT.ref_utilisateur = U.ref_utilisateur
WHERE
	U.pays = 'France'
GROUP BY
	LT.ref_liste
HAVING
	COUNT(TAL.ref_tache) >= 5;
...
FROM
	Utilisateur U
INNER JOIN Est_assigne EA ON
	U.ref_utilisateur = EA.ref_utilisateur
JOIN Tache_fini TF ON
	EA.ref_tache = TF.ref_tache
JOIN Score_categorie_tache S ON
	TF.nom_categorie = S.nom_categorie
WHERE
	S.termine = 'O'
GROUP BY
	U.nom_programme
...
FROM
	Utilisateur U
LEFT JOIN Est_assigne EA ON
	U.ref_utilisateur = EA.ref_utilisateur
LEFT JOIN Tache_en_cours TE ON
	EA.ref_tache = TE.ref_tache
LEFT JOIN Tache_fini TF ON
	EA.ref_tache = TF.ref_tache
GROUP BY
	U.login,
	U.nom,
...
SELECT
	...
	U.nom,
	U.prenom,
	SUM(SC.score) AS total_points_gagnes
FROM
	Utilisateur U
JOIN Est_assigne EA ON
	U.ref_utilisateur = EA.ref_utilisateur
JOIN Tache_fini TF ON
	EA.ref_tache = TF.ref_tache
JOIN Score_categorie_tache SC ON
	TF.nom_categorie = SC.nom_categorie
WHERE
